...
def create_cf_records():
    ...
    def create_dns_record(name, proxied):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to add DNS record. Status code: {response.status_code}, Error: {response.text}"
            )
            return None
...
if os.environ.get("CF_ENABLE", "") == "true":
    ...
    if create_cf_records() is not None:
        ...
        if not cf_only:
            ...
            with open(
                f"/root/.acme.sh/{direct_subdomain}_ecc/fullchain.cer", "r"
            ) as file:
                cert_public = file.read()
                cert_public = json.dumps(cert_public)[1:-1]
            with open(
                f"/root/.acme.sh/{direct_subdomain}_ecc/{direct_subdomain}.key", "r"
            ) as file:
...
with open("inbounds.json") as f:
    ...
    for inbound in configured_inbounds:
        if isinstance(inbound.get("link"), dict):
            inbound["link"] = (
                "vmess://"
                + base64.b64encode(json.dumps(inbound["link"]).encode()).decode()
            )
    ...
    "routing": {
        ...
        "rules": [
            {"type": "field", "inboundTag": ["api"], "outboundTag": "api"},
            {
                "type": "field",
                "outboundTag": "blocked",
                "ip": ["geoip:private", "geoip:ru", "ext:geoip_IR.dat:ir"],
            },
